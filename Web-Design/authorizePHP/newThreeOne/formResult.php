...
$row = "SELECT * FROM `users_database` WHERE `userLogin` = '". $login ."'";
$is_login_free = mysqli_query($link, $row);
$rowTwuo = "SELECT * FROM `users_database` WHERE `userEmail` = '". $email ."'";
$is_login_freeTwuo = mysqli_query($link, $rowTwuo);
$sql = "INSERT INTO users_database (firstName, lastName, userLogin, userEmail, phone, userGender, hash, comment) VALUES ('$userBD', '$userBDSurName', '$login', '$email', '$phone', '$gender', '$hash', '$comment')";
             ...
             if ($is_login_freeTwuo->num_rows > 0) {
               ...
               if (empty($_GET['name'])) {
                 ...
                 if (!preg_match("/^(([a-zA-Z' -]{1,40})|([а-яА-Я' -]{1,40}))$/u", $userBD)) {
                     print_r("Введите корректное имя.<br>Русские или английские буквы, пробел, знаки ' или -.<br>Ваше имя:  {$name}  <br><br>");
                     print_r("Через 5 сек. Вы автоматически отправитесь на форму регистрации.");
                     Header("Refresh: 5; registration.php");
                 } else {
